@Library('jenkins-library@feature/FWW-956/prepare-firefox-ext-to-pass-checks')
...
def pipeline = new org.js.AppArtifactsPipeline(
    ...
    preBuildCmds:               ['apt-get update && apt-get install zip jq -y && corepack enable && yarn install'],
    nexusFiles:                 [ '.zip'],
