...
export async function handleTransfer(
  ...
  try {
    ...
    web3Api.provider
      ...
      .on('receipt', function (receipt: ethers.providers.TransactionReceipt) {
...
export async function getEVMTransactionObject(
  ...
  transferAll: boolean,
  web3ApiMap: Record<string, EthProvider>
): Promise<[ethers.providers.TransactionRequest, number, string]> {
  const web3Api = web3ApiMap[networkKey];
  ...
  transactionObject.gasPrice = gasLimit;

  const estimateFee = gasPrice.toNumber() * gasLimit.toNumber();

  transactionObject.value = transferAll ? estimateFee : value;

  return [transactionObject, transactionObject.value, estimateFee.toString()];
}
...
export async function getERC20TransactionObject(
  ...
  if (transferAll) {
    ...
    transferValue = freeAmount.toNumber() || 0;
  }

  function generateTransferData(to: string, transferValue: number): string {
  ...
  if (transferAll) {
    transferValue = new BN(freeAmount).toNumber();
