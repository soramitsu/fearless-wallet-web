...
export async function validateWasmToken(
  contractAddress: string,
  tokenType: CustomTokenType.psp22 | CustomTokenType.psp34,
  ...
  try {
    if (tokenType === CustomTokenType.psp22) {
      ...
      if (
        !(nameResp.result.isOk && symbolResp.result.isOk && decimalsResp.result.isOk) ||
        !nameResp.output ||
        !decimalsResp.output ||
        !symbolResp.output
      ) {
