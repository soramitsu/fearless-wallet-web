...
import { ApiProps, BasicTxResponse, SignerType } from '../../background/types';
import { NetworkJson, TokenInfo } from '../evm/types/ether';
import { signAndSendExtrinsic } from '../substrate/shared/signAndSendExtrinsic';
import { getUnsupportedResponse } from '../substrate/transfer';
import { astarEstimateCrossChainFee, astarGetXcmExtrinsic } from './astar';
import { moonbeamEstimateCrossChainFee, moonbeamGetXcmExtrinsic } from './moonbeamXcm';
import { statemintEstimateCrossChainFee, statemintGetXcmExtrinsic } from './statemintXcm';
import { substrateEstimateCrossChainFee, substrateGetXcmExtrinsic } from './substrateXcm';
import { SupportedCrossChainsMap } from './utils';
...
export function isNetworksPairSupportedTransferCrossChain(
  ...
  if (
    !SupportedCrossChainsMap[originNetworkKey] ||
  ...
  if (
    SupportedCrossChainsMap[originNetworkKey].relationMap[destinationNetworkKey].type === 'p' &&
    !(networkMap[destinationNetworkKey] && networkMap[destinationNetworkKey].paraId)
  ) {
    return false;
  }
...
export async function estimateCrossChainFee(
  ...
  if (
    !isNetworksPairSupportedTransferCrossChain(originNetworkKey, destinationNetworkKey, tokenInfo.symbol, networkMap)
  ) {
    console.info('unsupported xcm');

    return ['0', ''];
  }

  if (['moonbase', 'moonriver', 'moonbeam'].includes(originNetworkKey)) {
    return moonbeamEstimateCrossChainFee(
      originNetworkKey,
  ...
  if (['astar', 'shiden'].includes(originNetworkKey)) {
    return astarEstimateCrossChainFee(
      originNetworkKey,
      destinationNetworkKey,
      to,
      fromKeypair,
  ...
  if (['statemint', 'statemine'].includes(originNetworkKey)) {
    return statemintEstimateCrossChainFee(
      originNetworkKey,
      destinationNetworkKey,
      to,
      fromKeypair,
  ...
  return substrateEstimateCrossChainFee(
    originNetworkKey,
    destinationNetworkKey,
    to,
    fromKeypair,
...
export const createXcmExtrinsic = async ({
  ...
  tokenInfo,
  value,
}: CreateXcmExtrinsicProps): Promise<SubmittableExtrinsic<'promise'>> => {
  ...
  if (['moonbase', 'moonriver', 'moonbeam'].includes(originNetworkKey)) {
    ...
    extrinsic = statemintGetXcmExtrinsic(
      originNetworkKey,
      destinationNetworkKey,
      to,
      value,
    ...
    extrinsic = substrateGetXcmExtrinsic(
      originNetworkKey,
      destinationNetworkKey,
      to,
      value,
...
export async function makeCrossChainTransfer({
  ...
  tokenInfo,
  value,
}: MakeCrossChainTransferProps): Promise<void> {
...
export function updateXcmResponseTxResult(
  ...
  for (let index = 0; index < records.length; index++) {
    ...
    if (['karura', 'acala', 'acala_testnet'].includes(networkKey) && tokenInfo && !tokenInfo.isMainToken) {
      ...
      if (record.event.section === 'tokens' && record.event.method.toLowerCase() === 'withdrawn') {
      ...
      if (record.event.section === 'tokens' && record.event.method.toLowerCase() === 'withdrawn') {
        response.txResult.change = record.event.data[2]?.toString() || '0';
        response.txResult.changeSymbol = tokenInfo.symbol;
      } else if (record.event.section === 'balances' && record.event.method.toLowerCase() === 'transfer') {
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        response.txResult.change = record.event.data[2]?.toString() || '0';
        response.txResult.changeSymbol = tokenInfo.symbol;
      }
    } else if (['astar', 'shiden'].includes(networkKey) && tokenInfo) {
      if (record.event.section === 'assets' && record.event.method.toLowerCase() === 'burned') {
        response.txResult.change = record.event.data[2]?.toString() || '0';
        response.txResult.changeSymbol = tokenInfo.symbol;
      }
