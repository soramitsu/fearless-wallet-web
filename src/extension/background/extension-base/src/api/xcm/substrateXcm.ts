...
import {
  ...
  SupportedCrossChainsMap,
  getMultiLocationFromParachain,
  getReceiverLocation,
} from './utils';
...
function getTokenIdentity(originNetworkKey: string, tokenInfo: TokenInfo) {
  ...
  const tokenSymbol =
    tokenInfo.symbol.toUpperCase() === 'AUSD' && originNetworkKey === 'karura'
      ? 'KUSD'
  ...
  } else if (originNetworkKey === 'karura' && tokenSymbol.toUpperCase() === 'NEER') {
    // TODO: modify later with different assets on Karura
    return tokenInfo.specialOption as Record<string, any>;
  } else if (originNetworkKey === 'acala' && tokenSymbol.toUpperCase() === 'GLMR') {
    // TODO: modify later with different assets on Acala
    return tokenInfo.specialOption as Record<string, any>;
  }
...
export async function substrateEstimateCrossChainFee(
  ...
  const weightParam = NETWORK_USE_UNLIMIT_WEIGHT.includes(originNetworkKey)
    ? POLKADOT_UNLIMITED_WEIGHT
    : FOUR_INSTRUCTIONS_WEIGHT;
  ...
  try {
    if (SupportedCrossChainsMap[originNetworkKey].type === 'p') {
      ...
      try {
        ...
        feeString =
          parseNumberToDisplay(paymentInfo.partialFee, originNetworkJson.decimals) +
          ` ${originNetworkJson.nativeToken ? originNetworkJson.nativeToken : ''}`;
      } catch (e) {
        feeString = `0.0000 ${originNetworkJson.nativeToken ? originNetworkJson.nativeToken : ''}`;
      }

      console.info('substrate xcm tx p-p or p-r here', extrinsic.toHex());
    } else {
      // Case RelayChain -> ParaChain
      const receiverLocation: Record<string, any> = getReceiverLocation(
        originNetworkKey,
      ...
      if (['statemint', 'statemine'].includes(destinationNetworkKey)) {
        ...
        console.info('substrate xcm teleport asset tx r-p here', extrinsic.toHex());
      } else {
        ...
        console.info('substrate xcm reserve transfer tx r-p here', extrinsic.toHex());
      }
    }
