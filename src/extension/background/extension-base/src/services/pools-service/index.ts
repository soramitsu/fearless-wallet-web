...
import { type PoolsParamsResponse } from './types';
...
export class PoolsService {
  ...
  public async getPoolsParams(params: StakingParamsRequest): Promise<PoolsParamsResponse> {
    ...
    const promises: Promise<StakingParams>[] = networks.map(async (network) => {
      ...
      const myStakingInfo = await this.getMyPoolsInfo(network, validators, minBond);
