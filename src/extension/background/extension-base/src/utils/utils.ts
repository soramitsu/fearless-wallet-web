...
export function mergeNetworkProviders(customNetwork: NetworkJson, predefinedNetwork: NetworkJson) {
...
export const filterAndSortingAccountByAuthType = (
  ...
  if (accountAuthType === 'substrate') {
    ...
    if (sorting) {
      rs.sort((acc, acc2) => {
        if (
          (acc.type === 'ethereum' && acc2.type === 'ethereum') ||
          (acc.type !== 'ethereum' && acc2.type !== 'ethereum')
        ) {
          return 0;
        } else {
