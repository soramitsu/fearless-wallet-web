...
import { options } from '@sora/options';
import { CodecString, FPNumber, NumberLike } from '@sora/math';
import { Connection } from '@sora/connection';
...
export class Api<T = void> extends BaseApi<T> {
  ...
  public async restoreActiveAccount(): Promise<void> {
    ...
    if (address) {
      ...
      let parsedExternal: boolean | null = null;

      if (isExternalFlag) {
        try {
          parsedExternal = JSON.parse(isExternalFlag) as boolean;
        } catch (error) {
          console.warn('Sora API: failed to parse external flag', error);
          this.storage?.remove('isExternal');
        }
      }

      const isExternalAccount = parsedExternal || (parsedExternal === null && !!source);
