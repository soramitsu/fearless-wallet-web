...
export class WithConnectionApi {
  ...
  public async getAccountOnChainIdentity(address: string): Promise<OnChainIdentity | null> {
    const data = (await this.api.query.identity.identityOf(address)) as unknown as OptionLike<any>;

    if (data.isEmpty || data.isSome === false) return null;
...
export class WithKeyring extends WithAccountPair {
  ...
  public getAccounts(): KeyringAddress[] {
    return keyring.getAccounts() as KeyringAddress[];
  }
...
export class WithAccountHistory extends WithAccountStorage {
  ...
  public get history(): AccountHistory<HistoryItem> {
    if (this.accountStorage) {
      ...
      if (history) {
        try {
          this._history = JSON.parse(history) as AccountHistory<HistoryItem>;
        } catch (error) {
          console.warn('WithAccountHistory: failed to parse account history', error);
          this.accountStorage.remove(this.historyNamespace);
          this._history = {};
        }
      } else {
        this._history = {};
  ...
  public saveHistory(historyItem: HistoryItem, options?: SaveHistoryOptions): void {
    ...
    if (needToUpdateAddressStorage) {
      ...
      if (history) {
        try {
          historyCopy = JSON.parse(history) as AccountHistory<HistoryItem>;
        } catch (error) {
          console.warn('WithAccountHistory: failed to parse signer history', error);
          addressStorage.remove(this.historyNamespace);
          historyCopy = {};
        }
      } else {
        historyCopy = {};
        ...
        if (result.status.isInBlock) {
          ...
          for (const {
            ...
            if (method === 'FeeWithdrawn' && section === 'xorFee') {
              ...
              const amountRaw = (amount as unknown as { toString(): string }).toString();
              const amountFormatted = new FPNumber(amountRaw).toString();
