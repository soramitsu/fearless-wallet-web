...
type BridgeTypesGenericNetworkId = Codec;
type BridgeTypesAssetKind = {
  readonly isSidechain: boolean;
};
type BridgeTypesGenericAccount = Codec;

const parseSubBridgeAssetKind = (kind: OptionLike<BridgeTypesAssetKind>): SubAssetKind => {
  return kind.unwrap().isSidechain ? SubAssetKind.Sidechain : SubAssetKind.Thischain;
};

const parseSubBridgeAssetDecimals = (precision: OptionLike<u8>): number => {
...
const isOptionLikeCodec = <T>(value: unknown): value is OptionLike<T> =>
  ...
  typeof (value as OptionLike<T>).isEmpty === 'boolean';
...
const toCodecArray = (value: unknown): unknown[] => {
  ...
  if (value && typeof (value as { values?: () => Iterable<unknown> }).values === 'function') {
...
const SUB_BRIDGE_PRECISION_MISSING = 'Substrate bridge precision option is not available';
const SUB_BRIDGE_ASSET_ID_MISSING = 'Substrate bridge asset id option is not available';
const SUB_BRIDGE_ASSET_KIND_MISSING = 'Substrate bridge asset kind option is not available';
const SUB_BRIDGE_RELAYCHAIN_ASSET_MISSING = 'Relaychain asset option is not available';
const SUB_BRIDGE_ALLOWED_ASSETS_MALFORMED = 'Allowed parachain assets response is malformed';
...
export class SubBridgeApi<T> extends ApiAccount<T> {
  ...
  public parachainIds: ParachainIds = {
    ...
    [SubNetworkId.KusamaMoonriver]: 2023,
  ...
  public async getSubAssetDecimals(subNetworkId: SubNetworkChainId, soraAssetId: string): Promise<number> {
    if (subNetworkId === SubNetworkId.Liberland) {
      const precisionCodec = await this.api.query.substrateBridgeApp.sidechainPrecision(subNetworkId, soraAssetId);
      const precision = ensureOptionLike<u8>(precisionCodec, SUB_BRIDGE_PRECISION_MISSING);

      return parseSubBridgeAssetDecimals(precision);
    } else {
      const precisionCodec = await this.api.query.parachainBridgeApp.sidechainPrecision(subNetworkId, soraAssetId);
      const precision = ensureOptionLike<u8>(precisionCodec, SUB_BRIDGE_PRECISION_MISSING);
  ...
  private async getSubAssetAddress(subNetworkId: SubNetworkChainId, soraAssetId: string): Promise<SubAssetId> {
    if (subNetworkId === SubNetworkId.Liberland) {
      const resultCodec = await this.api.query.substrateBridgeApp.sidechainAssetId(subNetworkId, soraAssetId);
      const result = ensureOptionLike<any>(resultCodec, SUB_BRIDGE_ASSET_ID_MISSING);
  ...
  private async getSubAssetKind(subNetworkId: SubNetworkChainId, soraAssetId: string): Promise<SubAssetKind> {
    if (subNetworkId === SubNetworkId.Liberland) {
      const kindCodec = await this.api.query.substrateBridgeApp.assetKinds(subNetworkId, soraAssetId);
      const kind = ensureOptionLike<BridgeTypesAssetKind>(kindCodec, SUB_BRIDGE_ASSET_KIND_MISSING);

      return parseSubBridgeAssetKind(kind);
    } else {
      const kindCodec = await this.api.query.parachainBridgeApp.assetKinds(subNetworkId, soraAssetId);
      const kind = ensureOptionLike<BridgeTypesAssetKind>(kindCodec, SUB_BRIDGE_ASSET_KIND_MISSING);
  ...
  private async getRelayChainAssets(relaychain: Relaychain): Promise<Record<string, SubAsset>> {
    ...
    try {
      const assetCodecRaw = await this.api.query.parachainBridgeApp.relaychainAsset(relaychain);
      const assetCodec = ensureOptionLike<any>(assetCodecRaw, SUB_BRIDGE_RELAYCHAIN_ASSET_MISSING);
  ...
  private async getParaChainAssets(parachain: Parachain): Promise<Record<string, SubAsset>> {
    ...
    try {
      const assetsRaw = await this.api.query.parachainBridgeApp.allowedParachainAssets(relaychain, parachainId);
      const assetsCodecs = toCodecArray(assetsRaw);
      if (!assetsCodecs.length && assetsRaw) {
        console.warn('[SubBridgeApi] allowedParachainAssets returned empty set', SUB_BRIDGE_ALLOWED_ASSETS_MALFORMED);
      }

      const soraAssetIds = assetsCodecs.reduce<string[]>((buffer, item) => {
        if (isAssetIdInput(item)) {
          buffer.push(toAssetId(item));
        } else {
          console.warn('[SubBridgeApi] Skipping malformed parachain asset entry', item);
        }
