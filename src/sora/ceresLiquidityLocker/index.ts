...
type LockerDataCodec = {
  poolTokens: unknown;
  unlockingTimestamp: { toNumber(): number };
  assetA: { toString(): string };
  assetB: { toString(): string };
};

type LockerDataCollection = {
  values(): Iterable<LockerDataCodec>;
};
...
export class CeresLiquidityLockerModule<T> {
  ...
  public getLockerDataObservable(onlyActual = true): Observable<AccountLockedPool[]> {
    ...
    return this.root.apiRx.query.ceresLiquidityLocker.lockerData(this.root.account.pair.address).pipe(
      map((lockedData) => {
        const values = Array.from((lockedData as unknown as LockerDataCollection).values());
        const infos = values.map((value) => {
          return {
            poolTokens: new FPNumber(codecToString(value.poolTokens)),
