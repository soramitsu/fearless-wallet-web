...
export class ExtendedAssetsModule<T> {
  ...
  public async getSbtMetaInfo(assetId: string): Promise<SoulBoundToken> {
    ...
    const specific =
      typeof (sbtSpecificInfo as any).unwrapOrDefault === 'function'
        ? (sbtSpecificInfo as any).unwrapOrDefault()
        : (sbtSpecificInfo as any);
    const common =
      typeof (sbtCommonInfo as any).toHuman === 'function'
        ? ((sbtCommonInfo as any).toHuman() as Partial<SoulBoundToken>)
        : ({} as Partial<SoulBoundToken>);

    const regulatedAssetsList = (specific.regulatedAssets ?? []).map(
      (assetAddressEncoded: any) => assetAddressEncoded?.code?.toString?.() ?? assetAddressEncoded?.toString?.() ?? ''
    );
    ...
    return {
      ...
      externalUrl: specific.externalUrl?.toHuman?.() ?? specific.externalUrl?.toString?.() ?? '',
      issuedAt: specific.issuedAt?.toHuman?.() ?? specific.issuedAt?.toString?.(),
  ...
  public async getAllSbtIds(): Promise<Array<string>> {
    ...
    return data.map(([key]) => (key.args?.[0] as any)?.code?.toString?.() ?? '');
  }
  ...
  public async getSbtExpiration(accountId: string, sbtAssetId: string): Promise<string> {
    const expiration = await this.root.api.query.extendedAssets.sbtExpiration(accountId, sbtAssetId);
    const value = (expiration as any)?.unwrapOr?.(Infinity) ?? Infinity;
  ...
  public async regulateAsset(assetId: string): Promise<T> {
    ...
    return this.root.submitExtrinsic(this.root.api.tx.extendedAssets.regulateAsset(assetId), account.pair, {
  ...
  public async bindRegulatedAssetToSBT(sbtAssetId: string, regulatedAssetIds: Array<string>): Promise<T> {
    ...
    return this.root.submitExtrinsic(this.root.api.tx.utility.batchAll(transactions), account.pair, {
