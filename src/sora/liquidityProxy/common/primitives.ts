  ...
  alignChunkPrecision(chunk: SwapChunk): [SwapChunk, SwapChunk] {
    ...
    if (precision) {
      if (precision.isInput) {
        if (!chunk.input.sub(precision.amount.mul(chunk.input.div(precision.amount).floor())).isZero()) {
        ...
        if (!chunk.output.sub(precision.amount.mul(chunk.output.div(precision.amount).floor())).isZero()) {
  ...
  public verify(): boolean {
    ...
    for (const chunk of this.chunks) {
      ...
      if (precision) {
        ...
        if (
          !chunk.input.sub(inputPrecision.mul(chunk.input.div(inputPrecision).floor())).isZero() ||
          !chunk.output.sub(outputPrecision.mul(chunk.output.div(outputPrecision).floor())).isZero()
        ) {
          return false;
        }
      }
