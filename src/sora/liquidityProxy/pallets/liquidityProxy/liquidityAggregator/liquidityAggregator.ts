...
export class LiquidityAggregator {
  ...
  public fitChunk(chunk: SwapChunk, source: LiquiditySourceTypes, amount: FPNumber): [SwapChunk, FPNumber] {
    ...
    if (!chunk.isZero()) {
      ...
      if (FPNumber.isLessThan(amount, step)) {
        ...
        if (!step.isZero()) {
          const ratio = safeDivide(amount, step);
          const count = Math.floor(ratio.toNumber());
          const aligned = FPNumber.fromNatural(count, amount.precision).mul(step);
          sideAmount = FPNumber.isEqualTo(aligned, amount)
            ? new SideAmount(amount, this.variant)
            : new SideAmount(aligned, this.variant);
        } else {
  ...
  public calculateResult(): AggregationResult {
    const distribution: DistributionChunk[] = [];
