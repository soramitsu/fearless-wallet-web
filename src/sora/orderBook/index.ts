...
import { OrderBookStatus, PriceVariant } from '@sora/liquidityProxy';
import type { OrderBook, OrderBookPriceVolume } from '@sora/liquidityProxy';
...
type BalanceUnitCodec = {
  inner: { toString(): string };
  isDivisible: { isTrue: boolean };
};

type PriceVariantCodec = {
  isBuy: boolean;
  isSell: boolean;
};

type OrderBookStatusCodec = {
  isTrade: boolean;
  isPlaceAndCancel: boolean;
  isOnlyCancel: boolean;
};

type OrderBookIdCodec = {
  dexId: { toNumber(): number };
  base: { toString(): string };
  quote: { toString(): string };
};

type OrderBookStructCodec = {
  orderBookId: OrderBookIdCodec;
  status: OrderBookStatusCodec;
  lastOrderId: { toNumber(): number };
  tickSize: BalanceUnitCodec;
  stepLotSize: BalanceUnitCodec;
  minLotSize: BalanceUnitCodec;
  maxLotSize: BalanceUnitCodec;
};

type LimitOrderCodec = {
  id: { toNumber(): number };
  expiresAt: { toNumber(): number };
  lifespan: { toNumber(): number };
  time: { toNumber(): number };
  owner: { toString(): string };
  side: PriceVariantCodec;
  price: BalanceUnitCodec;
  amount: BalanceUnitCodec;
  originalAmount: BalanceUnitCodec;
};

type AggregatedMapCodec = {
  forEach(cb: (value: BalanceUnitCodec, key: BalanceUnitCodec) => void): void;
};

type IdsCodec = {
  unwrapOrDefault(): { toJSON(): unknown };
};

type LimitOrderIdsCodec = {
  unwrapOrDefault(): { toJSON(): unknown };
};

type StorageKeyArgsCodec = {
  args: [unknown, { base: { toString(): string }; quote: { toString(): string } }];
};
...
function toFP(value: BalanceUnitCodec): FPNumber {
...
const getStatus = (status: OrderBookStatusCodec): OrderBookStatus => {
...
const getPriceVariant = (variant: PriceVariantCodec): PriceVariant => {
  if (variant.isBuy) {
...
const formatOrderBookOption = (option: OptionCodec<OrderBookStructCodec>): OrderBook | null => {
...
export class OrderBookModule<T> {
  ...
  public async getOrderBooks(): Promise<Record<string, OrderBook>> {
    ...
    const orderBooks = entries.reduce<Record<string, OrderBook>>((buffer, [_, value]) => {
      const book = formatOrderBookOption(value as unknown as OptionCodec<OrderBookStructCodec>);
  ...
  public async getUserOrderBooks(account: string): Promise<string[]> {
    ...
    return entries.map(([key, _]) => {
      const [, orderBookId] = key.args as unknown as StorageKeyArgsCodec['args'];
      return this.serializeKey(toAssetId(orderBookId.base), toAssetId(orderBookId.quote));
    });
  }
  ...
  public getOrderBookObservable(base: string, quote: string): Observable<OrderBook | null> {
    ...
    return this.root.apiRx.query.orderBook
      ...
      .pipe(map((option) => formatOrderBookOption(option as unknown as OptionCodec<OrderBookStructCodec>)));
  }
  ...
  public async getAggregatedAsks(base: string, quote: string, dex?: number): Promise<Array<OrderBookPriceVolume>> {
    ...
    const data = (await this.root.api.query.orderBook.aggregatedAsks({
      dexId,
      base,
      quote,
    })) as unknown as AggregatedMapCodec;
  ...
  public subscribeOnAggregatedAsks(base: string, quote: string, dex?: number): Observable<Array<OrderBookPriceVolume>> {
    ...
    return this.root.apiRx.query.orderBook.aggregatedAsks({ dexId, base, quote }).pipe(
      map((data) => {
        ...
        (data as unknown as AggregatedMapCodec).forEach((value, key) => {
          const price = toFP(key);
          const amount = toFP(value);
          asks.push([price, amount]);
        });
  ...
  public async getAggregatedBids(base: string, quote: string, dex?: number): Promise<Array<OrderBookPriceVolume>> {
    const dexId = dex ?? this.root.dex.getDexId(quote);
    const data = (await this.root.api.query.orderBook.aggregatedBids({
      dexId,
      base,
      quote,
    })) as unknown as AggregatedMapCodec;
  ...
  public async getUserLimitOrdersIds(base: string, quote: string, account: string): Promise<Array<number>> {
    const dexId = this.root.dex.getDexId(quote);
    const idsCodec = (await this.root.api.query.orderBook.userLimitOrders(account, {
      dexId,
      base,
      quote,
    })) as unknown as LimitOrderIdsCodec;
  ...
  public async getAllUserLimitOrdersIds(account: string): Promise<Array<OrderId>> {
    ...
    return data.flatMap(([key, value]) => {
      const [, orderBookId] = key.args as unknown as StorageKeyArgsCodec['args'];
      const baseId = toAssetId(orderBookId.base);
      const quoteId = toAssetId(orderBookId.quote);
      const ids = ((value as unknown as LimitOrderIdsCodec).unwrapOrDefault().toJSON() ?? []) as Array<number>;
  ...
  public subscribeOnUserLimitOrdersIds(base: string, quote: string, account: string): Observable<Array<number>> {
    ...
    return this.root.apiRx.query.orderBook
      ...
      .pipe(
        map(
          (idsCodec) => ((idsCodec as unknown as LimitOrderIdsCodec).unwrapOrDefault().toJSON() ?? []) as Array<number>
        )
      );
  }

  private formatLimitOrder(
    orderCodec: OptionCodec<LimitOrderCodec>,
    base: string,
    quote: string,
    dexId: number
  ): LimitOrder | null {
    if (orderCodec.isNone) return null;
  ...
  public async getLimitOrder(base: string, quote: string, id: number): Promise<LimitOrder | null> {
    ...
    const orderCodec = (await this.root.api.query.orderBook.limitOrders(
      { dexId, base, quote },
      id
    )) as unknown as OptionCodec<LimitOrderCodec>;
  ...
  public async isOrderPlaceable(base: string, quote: string, side: PriceVariant, price: string): Promise<boolean> {
    ...
    const idsRaw = (await query({ dexId, base, quote }, { inner, isDivisible: true })) as unknown as {
      unwrapOrDefault(): { toJSON(): unknown };
    };

    const ids = (idsRaw.unwrapOrDefault().toJSON() ?? []) as Array<unknown>;

    return ids.length < MAX_ORDERS_PER_SINGLE_PRICE;
  }
  ...
  public subscribeOnLimitOrder(base: string, quote: string, id: number): Observable<LimitOrder | null> {
    ...
    return this.root.apiRx.query.orderBook
      ...
      .pipe(
        map((orderCodec) =>
          this.formatLimitOrder(orderCodec as unknown as OptionCodec<LimitOrderCodec>, base, quote, dexId)
        )
      );
  }
