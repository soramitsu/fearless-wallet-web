...
import type { OptionLike, TupleLike, VecLike } from '@sora-types';
...
type AssetInput = Asset | AccountAsset;
type LiquidityTuple = [AssetInput, AssetInput, NumberLike, NumberLike, number];

type TradingPairRpcSection = {
  isPairEnabled(dexId: number, base: string, target: string): Promise<{ isTrue: boolean }>;
};

type PairCreationArgs = [number, string, string];
type DepositLiquidityArgs = [number, string, string, string, string, string, string];
type WithdrawLiquidityArgs = [number, string, string, string, string, string];

type TradingPairTxSection = {
  register(...args: PairCreationArgs): SubmittableExtrinsic<'promise'>;
};

type PoolXykTxSection = {
  initializePool(...args: PairCreationArgs): SubmittableExtrinsic<'promise'>;
  depositLiquidity(...args: DepositLiquidityArgs): SubmittableExtrinsic<'promise'>;
  withdrawLiquidity(...args: WithdrawLiquidityArgs): SubmittableExtrinsic<'promise'>;
};
...
function unwrapOption<T>(value: unknown): T | null {
  const option = value as OptionLike<T> | undefined;

  if (!option || option.isEmpty) return null;
  if (option.value !== undefined) return option.value as T;

  return option.unwrapOr(null);
}

function toReserve(value: unknown): CodecString {
  return new FPNumber(value as u128).toCodecString();
}

function parseReserves(reserves: unknown): [CodecString, CodecString] {
  const tuple = reserves as TupleLike<unknown, unknown> | null;

  if (!tuple || tuple.length !== 2) {
    return ['0', '0'];
  }

  return [toReserve(tuple[0]), toReserve(tuple[1])];
}
...
export class PoolXykModule<T> {
  ...
  private getPoolXykTx(): PoolXykTxSection {
    return this.root.api.tx.poolXYK as unknown as PoolXykTxSection;
  }

  private getTradingPairTx(): TradingPairTxSection {
    return this.root.api.tx.tradingPair as unknown as TradingPairTxSection;
  }

  private getTradingPairRpc(): TradingPairRpcSection | null {
    const rpc = this.root.api.rpc as unknown as Record<string, unknown>;

    return (rpc.tradingPair as TradingPairRpcSection) ?? null;
  }
  ...
  public async check(firstAssetAddress: string, secondAssetAddress: string): Promise<boolean> {
    const props = unwrapOption<unknown>(
  ...
  public getAccountPoolBalanceObservable(firstAddress: string, secondAddress: string): Observable<string | null> {
    ...
    return this.root.apiRx.query.poolXYK.poolProviders(poolAccount, this.root.account.pair.address).pipe(
      map((result) => {
        const value = unwrapOption<u128>(result);

        if (!value) return null;

        return toReserve(value);
      })
    );
  }
  ...
  public async getTotalSupply(firstAddress: string, secondAddress: string): Promise<CodecString | null> {
    ...
    const result = unwrapOption<u128>(await this.root.api.query.poolXYK.totalIssuances(poolAccount));

    if (!result) return null;

    return toReserve(result);
  }
  ...
  public getPoolPropertiesObservable(
    ...
    return this.root.apiRx.query.poolXYK.properties(firstAssetAddress, secondAssetAddress).pipe(
      map((result) => {
        const value = unwrapOption<VecLike<{ toString(): string }>>(result);

        if (!value) return null;

        return value.toArray().map((accountId) => accountId.toString());
      })
    );
  }
  ...
  public async getAllReserves(): Promise<Record<string, Array<CodecString>>> {
    ...
    for (const item of allReserves) {
      ...
      const tuple = item[1] as unknown as TupleLike<unknown, unknown> | undefined;

      if (!tuple || tuple.length !== 2) continue;

      reserves[toAssetId(key1) + toAssetId(key2)] = [toReserve(tuple[0]), toReserve(tuple[1])];
    }
  ...
  public estimatePoolTokensMinted(
    ...
    if (!poolToken) return ['0', '0'];

    const decimalsValue = poolToken.decimals ?? FPNumber.DEFAULT_PRECISION;
    const pts = FPNumber.fromCodecValue(totalSupply, decimalsValue);
    const result = FPNumber.min(aIn.mul(pts).div(a), bIn.mul(pts).div(b));
    const safeResult = result ?? FPNumber.ZERO;

    return [safeResult.toCodecString(), pts.toCodecString()];
  }
  ...
  private arrangeAssetsForParams(
    ...
    assert(
      baseAsset &&
        ...
        targetAssetAmount !== undefined &&
        DEXId !== undefined,
      Messages.xorOrXstIsRequired
    );
  ...
  public getUserPoolsSubscription(): Subscription {
    ...
    return this.root.apiRx.query.poolXYK.accountPools.multi(multiEntries).subscribe(async (lists) => {
      ...
      const listsArray = (lists as unknown as VecLike<VecLike<{ toString(): string }>>).toArray();

      listsArray.forEach((list, index) => {
        const baseAssetId = baseAssetIds[index];
        const targets = list.toArray();

        for (const targetAssetId of targets) {
  ...
  private calcAddTxParams(
    ...
    const args: DepositLiquidityArgs = [
      DEXId,
      firstAsset.address,
      secondAsset.address,
  ...
  public add(
    ...
    return this.root.submitExtrinsic(this.getPoolXykTx().depositLiquidity(...params.args), this.root.account.pair, {
  ...
  private async calcCreateTxParams(
    ...
    pairCreationArgs: PairCreationArgs;
    addLiquidityArgs: DepositLiquidityArgs;
    baseAssetAmount: NumberLike;
    targetAssetAmount: NumberLike;
  }> {
    const [baseAsset, targetAsset, baseAssetAmount, targetAssetAmount, DEXId] = this.arrangeAssetsForParams(
  ...
  public async create(
    ...
    const tradingPairRpc = this.getTradingPairRpc();
    const isPairAlreadyCreated =
      (await tradingPairRpc?.isPairEnabled(dexId, baseAddress, targetAddress))?.isTrue === true;
    const transactions: SubmittableExtrinsic<'promise'>[] = [];
    if (!isPairAlreadyCreated) {
      const tradingPairTx = this.getTradingPairTx();
      transactions.push(tradingPairTx.register(...params.pairCreationArgs));
    }
    const poolXykTx = this.getPoolXykTx();
    transactions.push(
      poolXykTx.initializePool(...params.pairCreationArgs),
      poolXykTx.depositLiquidity(...params.addLiquidityArgs)
    );
  ...
  private calcRemoveTxParams(
    ...
    totalSupply: CodecString,
    slippageTolerance: NumberLike = this.root.defaultSlippageTolerancePercent
  ): { args: WithdrawLiquidityArgs; amountA: string; amountB: string } {
    assert(this.root.account, Messages.connectWallet);
    ...
    const args: WithdrawLiquidityArgs = [
      dexId,
      firstAsset.address,
      secondAsset.address,
  ...
  public remove(
    ...
    return this.root.submitExtrinsic(this.getPoolXykTx().withdrawLiquidity(...params.args), this.root.account.pair, {
