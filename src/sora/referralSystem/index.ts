...
export class ReferralSystemModule<T> {
  ...
  public async getInvitedUsers(referrerId: string): Promise<Array<string>> {
    const invited = (await this.root.api.query.referrals.referrals(referrerId)) as Vec<AccountId>;

    return invited.map((accountId) => accountId.toString());
  }
  ...
  public subscribeOnInvitedUsers(referrerId: string): Observable<Array<string>> {
    return this.root.apiRx.query.referrals
      ...
      .pipe(map((data) => (data as Vec<AccountId>).map((accountId) => accountId.toString())));
  }
  ...
  public async setInvitedUser(referrerId: string): Promise<T> {
    ...
    const bondedData = (await this.root.api.query.referrals.referrerBalances(referrerId)) as Option<u128>;
    const bondedValue = bondedData.isSome ? bondedData.unwrap().toString() : '0';
    const bonded = new FPNumber(bondedValue);
  ...
  public async hasEnoughXorForFee(accountId: string): Promise<boolean> {
    const bondedData = (await this.root.api.query.referrals.referrerBalances(accountId)) as Option<u128>;
    const bondedValue = bondedData.isSome ? bondedData.unwrap().toString() : '0';
    const bonded = new FPNumber(bondedValue);
    const requiredFeeValue = FPNumber.fromCodecValue(this.root.NetworkFee.ReferralSetInvitedUser || 0);
