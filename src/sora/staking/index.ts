...
import { LiquiditySourceTypes } from '@sora/liquidityProxy';
...
import type { CodecString, NumberLike } from '@sora/math';
...
import {
  ActiveEraCodec,
  BalanceCodec,
  LedgerUnlockChunkCodec,
  NumberCodec,
  OptionCodec,
  RewardDestinationCodec,
  RewardPointsCodec,
  StakingLedgerCodec,
  ValidatorPrefsCodec,
  ValidatorExposureCodec,
  optionIsEmpty,
  asOptionCodec,
  StringCodec,
} from './codecHelpers';
...
export class StakingModule<T> {
  ...
  private async resolveRewardToStakeRatio(): Promise<FPNumber> {
    try {
      const { amount } = await this.root.swap.getResultRpc(
        VAL.address,
        XOR.address,
        1,
        false,
      ...
      const ratio = FPNumber.fromCodecValue(amount);

      if (!ratio.isZero()) return ratio;
    } catch (error) {
      console.warn('[StakingModule] Preferred reward/stake quote failed, falling back to legacy RPC', error);
    }

    try {
      const { amount } = await this.root.swap.getResultFromDexRpc(VAL.address, XOR.address, 1);

      return FPNumber.fromCodecValue(amount);
    } catch (error) {
      console.error('[StakingModule] Unable to resolve reward/stake ratio', error);
  ...
  public getBondingDuration(): number {
    return (this.root.api.consts.staking.bondingDuration as unknown as NumberCodec).toNumber();
  }
  ...
  public getMaxNominatorRewardedPerValidator(): number {
    return (this.root.api.consts.staking.maxNominatorRewardedPerValidator as unknown as NumberCodec).toNumber();
  }
  ...
  public getMaxNominations(): number {
    return (this.root.api.consts.staking.maxNominations as unknown as NumberCodec).toNumber();
  }
  ...
  public async getMinNominatorBond(): Promise<number> {
    const bond = (await this.root.api.query.staking.minNominatorBond()) as unknown as NumberCodec;
    return bond.toNumber();
  }
  ...
  public getCurrentSessionObservable(): Observable<number> {
    return this.root.apiRx.query.session
      .currentIndex()
      .pipe(map((data) => (data as unknown as NumberCodec).toNumber()));
  }
  ...
  public getActiveEraObservable(): Observable<ActiveEra | null> {
    return this.root.apiRx.query.staking.activeEra().pipe(
      map((data) => {
        const option = asOptionCodec<ActiveEraCodec>(data as Option<any>);

        if (optionIsEmpty(option)) return null;

        const era = option.unwrap();
  ...
  public getHistoryDepth(): number {
    const historyDepth = this.root.api.consts.staking.historyDepth as unknown as NumberCodec;
  ...
  public async getCurrentEra(): Promise<number> {
    const data = (await this.root.api.query.staking.currentEra()) as unknown as Option<u32>;
  ...
  public getCurrentEraObservable(): Observable<number> {
    return this.root.apiRx.query.staking.currentEra().pipe(map((era) => formatEra(era as unknown as Option<u32>)));
  }
  ...
  public async getEraTotalStake(eraIndex: number): Promise<CodecString> {
    const erasTotalStake = (await this.root.api.query.staking.erasTotalStake(eraIndex)) as unknown as BalanceCodec;
  ...
  public getEraTotalStakeObservable(eraIndex: number): Observable<CodecString> {
    return this.root.apiRx.query.staking
      .erasTotalStake(eraIndex)
      .pipe(map((data) => (data as unknown as BalanceCodec).toString()));
  }
  ...
  public async getEraRewardPoints(eraIndex: number): Promise<RewardPointsIndividual> {
    ...
    return formatIndividualRewardPoints(data as unknown as RewardPointsCodec);
  }
  ...
  public getEraRewardPointsObservable(eraIndex: number): Observable<EraRewardPoints> {
    return this.root.apiRx.query.staking.erasRewardPoints(eraIndex).pipe(
      map((data) => {
        const codec = data as unknown as RewardPointsCodec;
        const total = codec.total.toNumber();
        const individual = formatIndividualRewardPoints(codec);
  ...
  public getAccountLedgerObservable(controllerAddress: string): Observable<AccountStakingLedger | null> {
    return this.root.apiRx.query.staking.ledger(controllerAddress).pipe(
      map((codec) => {
        const option = asOptionCodec<StakingLedgerCodec>(codec as Option<any>);
  ...
  public async getStashByController(controllerAddress: string): Promise<string> {
    const codec = asOptionCodec<StakingLedgerCodec>(
      (await this.root.api.query.staking.ledger(controllerAddress)) as Option<any>
    );

    if (optionIsEmpty(codec)) return '';
  ...
  public getControllerObservable(stashAddress: string): Observable<string | null> {
    return this.root.apiRx.query.staking.bonded(stashAddress).pipe(
      map((data) => {
        const option = asOptionCodec<{ toString(): string }>(data as Option<any>);
        if (optionIsEmpty(option)) return null;
        return option.unwrap().toString();
      })
    );
  }
  ...
  public getPayeeObservable(stashAddress: string): Observable<StakingRewardsDestination | string> {
    return this.root.apiRx.query.staking.payee(stashAddress).pipe(
      map((data) => {
        const payee = data as unknown as RewardDestinationCodec;

        if (payee.isStaked) return StakingRewardsDestination.Staked;
        if (payee.isController) return StakingRewardsDestination.Controller;
        if (payee.isStash) return StakingRewardsDestination.Stash;
        if (payee.isAccount) return payee.value.toString();
  ...
  public async getPayee(stashAddress: string): Promise<StakingRewardsDestination | string> {
    const payee = (await this.root.api.query.staking.payee(stashAddress)) as unknown as RewardDestinationCodec;
  ...
  public async getWannabeValidators(): Promise<ValidatorInfo[]> {
    const validators = (await this.root.api.query.staking.validators.entries()).map(([key, codec]) => {
      const address = (key.args[0] as StringCodec).toString();
      const prefs = codec as unknown as ValidatorPrefsCodec;
  ...
  public async getAverageRewards(eraIndex?: number): Promise<FPNumber> {
    ...
    if (eraIndex && Number.isInteger(eraIndex)) {
      const erasValidatorReward = (await erasValidatorRewardPallet(eraIndex)) as unknown as { value: CodecString };

      return new FPNumber(erasValidatorReward.value.toString());
    }
    ...
    const summaryRewards = erasValidatorReward.reduce((sum, [, eraReward]) => {
      const reward = eraReward as unknown as { value: CodecString };
      const eraRewardValue = reward.value.toString();
  ...
  public async calculateApy(
    ...
    if (!validatorTotalStake.value.isFinite() || validatorTotalStake.isZero()) {
      return '0';
    }
  ...
  public async getValidatorsInfo(): Promise<ValidatorInfoFull[]> {
    ...
    const rewardToStakeRatio = (await this.resolveRewardToStakeRatio()).toCodecString();
  ...
  public async getElectedValidators(eraIndex: number, clipped = false): Promise<ElectedValidator[]> {
    ...
    const validators = (await storage.entries(eraIndex)).map(([key, codec]) => {
      ...
      const data = formatValidatorExposure(codec as unknown as ValidatorExposureCodec);
  ...
  public getElectedValidatorObservable(
    ...
    return observable(eraIndex, validatorAddress).pipe(
      map((exposure) => formatValidatorExposure(exposure as unknown as ValidatorExposureCodec))
    );
  }
  ...
  public async getSessionValidators(): Promise<string[]> {
    const data = (await this.root.api.query.session.validators()) as unknown as StringCodec[];
  ...
  public async getNominations(stashAddress: string): Promise<StashNominatorsInfo | null> {
    const codec = asOptionCodec((await this.root.api.query.staking.nominators(stashAddress)) as Option<any>);
  ...
  public getNominationsObservable(stashAddress: string): Observable<StashNominatorsInfo | null> {
    return this.root.apiRx.query.staking
      .nominators(stashAddress)
      .pipe(map((codec) => formatNominations(asOptionCodec(codec as Option<any>))));
  }
  ...
  private async getSlashingSpans(stashAddress: string): Promise<u32> {
    const wrapper = (await this.root.api.query.staking.slashingSpans(stashAddress)) as unknown as {
      value: { spanIndex: u32 };
    };

    return wrapper.value.spanIndex;
  }
